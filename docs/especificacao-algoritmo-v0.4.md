...
### Versatilidade não é um componente da nota

O recomendador não premia um campeão por possuir muitas funções possíveis. Uma capacidade fica inerte até existir no draft uma ameaça, vulnerabilidade ou necessidade concreta que ela resolva.

```text
capacidade sem condição aplicável = 0 ponto
safe blind com laner conhecido = 0 ponto
função já coberta pelo time = 0 ponto
resposta incompatível com a ameaça = 0 ponto
```

Exemplos:

- `engage` não pontua apenas porque o time não possui outro iniciador; precisa existir um plano ou alvo que esse engage realmente alcance;
- `anti-dive` só pontua diante de uma entrada inimiga aplicável e perde valor se o time já a neutraliza;
- `frontline` não pontua contra uma composição que ignora ou derrete aquele perfil antes de ele ocupar espaço;
- `poke` só pontua contra um alvo ou plano vulnerável ao alcance do candidato;
- uma build com seis tags não soma seis bônus: para cada ameaça ou necessidade vale a melhor resposta aplicável, com uma segunda contribuição muito reduzida quando ela resolver outro problema distinto.
...
- Segurança de blind nunca gera bônus. Ela apenas reduz uma penalidade causada por informação faltante.
- Com laner e todos os inimigos conhecidos, o componente de blind é exatamente zero para qualquer campeão.
...
- O confronto da lane usa apenas relações de lane: troca, all-in, sustain, alcance, wave, recursos, CC realmente aplicável e exposição a gank.
- Engage de teamfight, peel, frontline e outras funções coletivas não geram ponto dentro do matchup só por fazerem parte do kit.
- Evidência específica revisada prevalece sobre a inferência por categorias.
...
O cálculo é feito contra os campeões inimigos realmente visíveis, não contra uma checklist abstrata. Para cada inimigo, o motor seleciona a melhor resposta aplicável e o maior risco de execução. No máximo duas ameaças distintas entram no bloco, com peso reduzido para a segunda. Adicionar uma capacidade que não interage com nenhum inimigo deve produzir diferença exatamente igual a zero.

### Composição aliada

O motor observa as ferramentas aliadas e as ameaças inimigas para ativar necessidades causais, por exemplo:

- dive inimigo e proteção insuficiente ativam necessidade de anti-dive/peel/disengage;
- poke inimigo e falta de acesso ativam engage ou backline access;
- frontline inimiga e falta de dano aplicável ativam DPS/anti-tank;
- mobilidade inimiga e pouco controle ativam CC/zoneamento;
- entrada aliada sem conversão ativa follow-up/burst;
- excesso claro de um tipo de dano ativa correção de dano.

As necessidades críticas são escolhidas antes de olhar o candidato. Apenas as duas maiores podem pontuar e a segunda tem peso reduzido. Assim, o campeão não cria para si próprio uma lista de qualidades convenientes. O arquétipo da composição pode continuar como explicação, mas não concede nota por si só.

### Separação semântica dos dados

`identidade/entrega`, `cenário buscado` e `riscos` são campos diferentes:

- entrega gera capacidades;
- cenário buscado gera condições de aplicabilidade contra o oponente;
- risco gera vulnerabilidades.

O texto "busca engage previsível" não pode criar uma tag de engage no próprio campeão. A base final será estruturada e revisada; tradução por palavras-chave existe apenas no protótipo exploratório e não é fonte suficiente para produção.
...
11. Draft inimigo completo produz componente de blind igual a zero para todos.
12. Adicionar uma capacidade irrelevante não altera matchup, composição inimiga nem composição aliada.
13. Perfil versátil sem interação aplicável recebe zero no bloco correspondente.
14. As necessidades aliadas são selecionadas sem consultar o candidato e ficam limitadas às duas mais críticas.
